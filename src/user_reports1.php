              ...
              <div class="card-body">
                ...
                <nav>
				  <div class="nav nav-tabs" id="nav-tab" role="tablist">
					<button class="nav-link active" id="nav-home-tab" data-bs-toggle="tab" data-bs-target="#nav-home" type="button" role="tab" aria-controls="nav-home" aria-selected="true">Today Summary</button>
					<button class="nav-link" id="nav-ovrlsummry-tab" data-bs-toggle="tab" data-bs-target="#nav-ovrlsummry" type="button" role="tab" aria-controls="nav-ovrlsummry" aria-selected="false">Overall Summary</button>
                      <button class="nav-link" id="nav-move-tab" data-bs-toggle="tab" data-bs-target="#nav-move" type="button" role="tab" aria-controls="nav-ovrlsummry" aria-selected="false">Movement Summary</button>
				...
				<div class="tab-content" id="nav-tabContent">
				  ...
				  <div class="tab-pane fade show active" id="nav-home" role="tabpanel" aria-labelledby="nav-home-tab">
                    ...
                    <tbody>
                      ...
                      (select count(*) from FF_REPORT2 x where   x.ENTERD_BY = a.ENTERD_BY and to_char(ENTERTED_ON,'yyyy/mm/dd') = '".$curbdate."') A5 , 
                      (select count(*) from FF_REPORT2 x where  x.SALES_CATAGORY = 'Future Sale' and x.ENTERD_BY = a.ENTERD_BY and to_char(ENTERTED_ON,'yyyy/mm/dd') = '".$curbdate."') A1 ,
                      (select count(*) from FF_REPORT2 x where  x.SALES_CATAGORY = 'Closed Sale' and x.ENTERD_BY = a.ENTERD_BY and to_char(ENTERTED_ON,'yyyy/mm/dd') = '".$curbdate."') A2 ,
                      (select count(*) from FF_REPORT2 x where  x.SALES_CATAGORY = 'Not Interested' and x.ENTERD_BY = a.ENTERD_BY and to_char(ENTERTED_ON,'yyyy/mm/dd') = '".$curbdate."') A3 ,
                      (select count(*) from FF_REPORT2 x where  x.SALES_CATAGORY is null and x.ENTERD_BY = a.ENTERD_BY and to_char(ENTERTED_ON,'yyyy/mm/dd') = '".$curbdate."') A4 
                      from FF_REPORT2 a
				  ...
				  <div class="tab-pane fade" id="nav-ovrlsummry" role="tabpanel" aria-labelledby="nav-ovrlsummry-tab">
                    ...
                    <div class="tab-pane fade" id="nav-move" role="tabpanel" aria-labelledby="nav-move-tab">
                        ...
                        <div class="table-responsive">
                            <table class="cell-border" id="tbl_move">
                                <thead>
                                <th>Team Office</th>
                                <th>Team</th>
                                <th>Service No</th>
								...
								$cdate = date('Y-m-d');
                                $sql = "SELECT (select REGEXP_SUBSTR((select  TEAM from FF_TEAM where sno = SID ), '[^-]+', 1, 1) from  dual) RTO,(select  TEAM from FF_TEAM where sno = SID ) TEAM,SID,USR_NAME,MOBILENO, TO_CHAR(LOG_DATE,'YYYY-MM-DD HH24:MI:SS') as LOG_DATE , (sysdate - LOG_DATE ) * 24 *60  UTIME 
                                FROM ( SELECT a.SID,b.USR_NAME,b.MOBILENO, a.LAT, a.LON, a.LOG_DATE, MAX(a.LOG_DATE) OVER (partition by a.SID) LATEST_DATE FROM FF_USER_LOCATIONS a,FF_APP_USERS b where A.SID = B.USR_ID)
                                ...
                                while ($row = oci_fetch_array($stid)) {
                                    ...
                                    <tr>
                                        ...
                                        <?php if ($row['UTIME'] > 60){ ?>
                                            <td bgcolor="red"><?php echo number_format($row['UTIME']) ?></td></tr>
                                        <?php }else if ($row['UTIME'] < 12){?>
                                        <td bgcolor="#32cd32"><?php echo number_format($row['UTIME']) ?></td></tr>
                                        <?php }else {?>
                                        <td bgcolor="yellow"><?php echo number_format($row['UTIME']) ?></td></tr>
                                        <?php } ?>
